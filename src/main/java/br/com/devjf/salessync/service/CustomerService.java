...
public class CustomerService {
    ...
    public boolean updateCustomer(Customer customer) {
        ...
        Customer existingCustomer = customerDAO.findByTaxId(customer.getTaxId());
        if (existingCustomer != null && !existingCustomer.getId().equals(
                customer.getId())) {
            return false;
        }
    ...
    public List<Customer> listCustomersWithPagination(int page, int pageSize) {
        return customerDAO.findWithPagination(page,
                pageSize);
    }
    ...
    public List<Customer> searchCustomersByName(String namePattern, int page, int pageSize) {
        return customerDAO.findByNameWithPagination(namePattern,
                page,
                pageSize);
    }
     ...
     *
     * @param criteria Map containing search criteria (name, taxId, email,
     * phone)
     * @param page The page number (starting from 0)
     * @param pageSize The number of records per page
    ...
    public List<Customer> searchCustomers(Map<String, String> criteria, int page, int pageSize) {
        ...
        if (criteria.containsKey("name") && !criteria.get("name").trim().isEmpty()) {
            return searchCustomersByName(criteria.get("name"),
                    page,
                    pageSize);
        }
        ...
        return listCustomersWithPagination(page,
                pageSize);
    }
    ...
    public String formatTaxId(String taxId) {
        ...
        if (digits.length() <= 11) {
            if (digits.length() < 11) {
                ...
                digits = String.format("%011d",
                        Long.valueOf(digits));
            }
            return digits.replaceAll("(\\d{3})(\\d{3})(\\d{3})(\\d{2})",
                    "$1.$2.$3-$4");
        } // Format as CNPJ (xx.xxx.xxx/xxxx-xx)
        else {
            if (digits.length() < 14) {
                // Pad with zeros if needed
                digits = String.format("%014d",
                        Long.valueOf(digits));
            }
            return digits.replaceAll("(\\d{2})(\\d{3})(\\d{3})(\\d{4})(\\d{2})",
                    "$1.$2.$3/$4-$5");
        }
    }
    ...
    public String formatPhone(String phone) {
        ...
        String digits = phone.replaceAll("[^0-9]",
                "");
        // Format based on length using switch case
        switch (digits.length()) {
            case 8:
                // Landline without area code: XXXX-XXXX
                return digits.replaceAll("(\\d{4})(\\d{4})",
                        "$1-$2");
            case 9:
                // Mobile without area code: XXXXX-XXXX
                return digits.replaceAll("(\\d{5})(\\d{4})",
                        "$1-$2");
            case 10:
                // Landline with area code: (XX) XXXX-XXXX
                return digits.replaceAll("(\\d{2})(\\d{4})(\\d{4})",
                        "($1) $2-$3");
            case 11:
                // Mobile with area code: (XX) XXXXX-XXXX
                return digits.replaceAll("(\\d{2})(\\d{5})(\\d{4})",
                        "($1) $2-$3");
            default:
                // If it doesn't match standard formats, apply basic formatting
                if (digits.length() > 8) {
                    // Try to extract area code if possible
                    return digits.replaceAll("(\\d{2})(\\d+)",
                            "($1) $2");
                } else {
                    return digits;
                }
