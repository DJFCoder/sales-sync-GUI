...
public class CustomerService {
    ...
    public Map<String, Object> getCompleteHistory(Integer customerId) {
        ...
        history.put("customer",
                customer);
        List<Sale> sales = saleDAO.findByCustomer(customer);
        history.put("sales",
                sales);
        List<ServiceOrder> serviceOrders = serviceOrderDAO.findByCustomer(
                customer);
        history.put("serviceOrders",
                serviceOrders);
        return history;
    }
    ...
    public List<Customer> searchCustomers(String searchTerm) {
        ...
        return allCustomers.stream()
                .filter(customer
                        -> (customer.getName() != null && customer.getName().toLowerCase().contains(
                term))
                || (customer.getTaxId() != null && customer.getTaxId().contains(
                term))
                || (customer.getEmail() != null && customer.getEmail().toLowerCase().contains(
                term))
                || (customer.getPhone() != null && customer.getPhone().contains(
                term))
                || (customer.getAddress() != null && customer.getAddress().toLowerCase().contains(
                term))
                )
    ...
    public Map<String, Object> getCustomerStatistics(Integer customerId) {
        ...
        statistics.put("customer",
                customer);
        // Get all sales for this customer
        List<Sale> sales = saleDAO.findByCustomer(customer);
        statistics.put("totalSales",
                sales.size());
        // Calculate total revenue from sales
        double totalRevenue = sales.stream()
                .mapToDouble(Sale::getTotalAmount)
                .sum();
        statistics.put("totalRevenue",
                totalRevenue);
        // Get all service orders for this customer
        List<ServiceOrder> serviceOrders = serviceOrderDAO.findByCustomer(
                customer);
        statistics.put("totalServiceOrders",
                serviceOrders.size());
        // Calculate average sale value
        statistics.put("averageSaleValue",
                sales.isEmpty() ? 0 : totalRevenue / sales.size());
        // Get last purchase date
        sales.stream()
                .max((s1, s2) -> s1.getDate().compareTo(s2.getDate()))
                .ifPresent(sale -> statistics.put("lastPurchaseDate",
                sale.getDate()));
        return statistics;
    }
