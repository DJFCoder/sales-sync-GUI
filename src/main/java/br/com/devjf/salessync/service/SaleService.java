...
public class SaleService {
    ...
    public Map<String, Double> calculateCommissions(Sale sale, User user) {
        ...
        if (user.getType() != UserType.EMPLOYEE) {
            commissions.put("commission", 0.0);
            return commissions;
        }
        ...
        commissions.put("rate", commissionRate);
        commissions.put("amount", commissionAmount);
    ...
    /**
     * Verifica se o usuário tem permissão para cancelar uma venda
     * Apenas usuários do tipo OWNER podem cancelar vendas
     */
    ...
    public double calculateTotalSalesByPeriod(LocalDate startDate, LocalDate endDate) {
        List<Sale> sales = getSalesByPeriod(startDate, endDate);
    ...
    public Map<String, Object> generateSalesSummary(LocalDate startDate, LocalDate endDate) {
        ...
        for (Sale sale : sales) {
            ...
            revenueByPaymentMethod.merge(method, sale.getTotalAmount(), Double::sum);
        }
        ...
        summary.put("revenueByPaymentMethod", revenueByPaymentMethod);
